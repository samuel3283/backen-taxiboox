package pe.com.microservice.taxiboot.web.filter;
...
import pe.com.microservice.taxiboot.core.util.ErrorCodesEnum;
...
import pe.com.microservice.taxiboot.service.util.Constants;
...
/*
							if (!isValidSession(session)) {
								...
								handleError(ErrorCodesEnum.ERR_SESSION_EXPIRED.getCode(),
										ErrorCodesEnum.ERR_SESSION_EXPIRED.getMessage(), res);
							} else {
	...
	*/
			...
			logger.info(ErrorCodesEnum.ERR_REQUEST_INVALID.getMessage());
	...
	private static boolean isValidSession(Session userInfo) throws ParseException {
		...
		/* if ((now - userInfo.getLoginTimestamp()) >= sessionTimeout) {
			logger.info("Sesion expirada, por vencimiento de tiempo límite de navegación al usuario: "
	...
	/**
	 * Obtiene el tiempo de expiraciÃ³n de sesiÃ³n.
	 * 
	 * @return tiempo de expiraciÃ³n de sesiÃ³n.
	 */
	private static long getSessionTimeout() {
		long timeout = timeOut;
		// verifica que se cumpla el mÃ­nimo.
	...
	/**
	 * Obtiene el tiempo de inactividad de sesiÃ³n.
	 * 
	 * @return tiempo de inactividad de sesiÃ³n.
	 */
	private static long getSessionInactiveTimeout() {
		long timeout = timeOutInac;
		// verifica que se cumpla el mÃ­nimo.
		if (timeout < MIN_INACTIVE_TIMEOUT) {
			timeout = MIN_INACTIVE_TIMEOUT;
		}
		return timeout;
	}
